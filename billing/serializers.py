...
class POSCreateSerializer(serializers.Serializer):
    ...
    # Optional: if provided, we’ll link the INVOICE to this customer (Order will still belong to cashier)
    customer_user_id = serializers.IntegerField(required=False, allow_null=True)

    # Optional shipping snapshot for Order (POS address can be minimal)
    ...
    def create(self, validated_data):
        request = self.context.get("request")
        cashier = request.user  # <-- the staff user creating the POS bill (required by permission)
        ...
        # Optional: link INVOICE to a known customer account (Order will still belong to cashier)
        ...
        # ---- Create ORDER + ITEMS owned by CASHIER ----
        order = Order.objects.create(
            user=cashier,                     # <-- OWNED BY CASHIER (auditable "who billed")
        ...
        # Lock & map products
        product_ids = [int(i["product_id"]) for i in items if i.get("product_id") is not None]
        ...
        for it in items:
            ...
            if pid is None:
                raise serializers.ValidationError({"items": "product_id is required for each line."})
        ...
        # Keep gross in order.total_amount (matches your online flow)
        ...
        inv = BillingInvoice.objects.create(
            ...
            status=BillingInvoice.STATUS_OPEN,
            order=order,
            customer=invoice_customer,                         # optional real customer linkage
